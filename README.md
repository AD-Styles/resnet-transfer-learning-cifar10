...
4. **시스템 리소스 최적화** — OOM(Out of Memory) 방지를 위한 배치 사이즈 및 데이터 파이프라인 제어
...
Stage A(헤드 단독 학습)만으로도 90%를 상회하는 정확도를 확보했습니다. 이는 224x224 해상도의 ImageNet으로 학습된 가중치가 32x32 저해상도인 CIFAR-10 데이터셋에서도 매우 강력한 **범용 특징 추출기(Feature Extractor)**로 작동함을 보여줍니다. 이후 진행된 Stage B(파인튜닝)는 상위 30개 레이어의 미세 조정을 통해 최종 정확도를 93% 이상으로 끌어올렸니다.
...
특히 기억에 남는 것은 단일 GPU(NVIDIA P100, 16GB) 환경에서 ResNet101 벤치마크 수행 중 직면한 OOM(Out Of Memory) 에러를 해결한 과정입니다. 초기 BATCH_SIZE=64 설정이 무거운 두 모델을 연달아 처리하며 VRAM 한계를 초과함을 디버깅을 통해 파악했습니다. 이에 훈련 데이터의 배치 사이즈를 32로 조정하여 메모리 여유를 확보하고, 추론(Inference) 벤치마크 단계에서는 tf.data 파이프라인을 unbatch().batch(16)으로 동적 재구성하여 메모리 파편화 이슈를 원천 차단했습니다. 이 경험을 통해 가용 하드웨어 리소스에 맞춰 파이프라인을 최적화하는 엔지니어링 역량의 중요성을 다시 한번 확인했습니다.

엔지니어링 측면에서 Learning Rate(LR)의 극단적인 단차 설계 또한 큰 깨달음을 주었습니다. Classifier 헤드만 학습하는 Stage A에서는 1e-3의 다소 높은 LR을 적용했지만, Backbone의 일부를 개방한 Stage B에서는 5e-6으로 200배나 낮춰야 했습니다. 이는 "미세 조정을 할 때는 LR을 단순히 작게 설정해야 한다"는 피상적인 암기를 넘어, ImageNet 대규모 데이터로 이미 훌륭한 최적점(Global Minimum 근처)에 도달해 있는 가중치 공간을 무너뜨리지 않기 위한 필수적인 수학적 조치임을 이해하게 되었습니다.
